...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "PUT") {
      const { market_key, weight } = req.body || {};
      ...
      const payload = { market_key, weight, updated_at: new Date().toISOString() };

      if (existing.ok && existing.json) {
        const full = { ...existing.json, weight, updated_at: payload.updated_at };
